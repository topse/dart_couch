...
/// Strips [AttachmentInfo.encoding] from serialized JSON.
///
/// [encoding] is an internal field that tracks whether CouchDB compressed
/// the attachment. It must not be sent to CouchDB (unknown stub fields are
/// harmless but add noise) and is not part of the standard attachment stub
/// format. The Dart field remains accessible for application/test code.
class AttachmentInfoRawHook extends MappingHook {
  const AttachmentInfoRawHook();

  @override
  Object? afterEncode(Object? value) {
    if (value is Map) value.remove('encoding');
    return value;
  }
}

@MappableClass(ignoreNull: true, hook: AttachmentInfoRawHook())
class AttachmentInfo with AttachmentInfoMappable {
  ...
  ///
  /// `null` for locally-created attachments, where [digest] = MD5(raw bytes)
  /// and the stored file content matches the digest.
  ///
  /// `'gzip'` when CouchDB compressed the attachment: [digest] =
  /// MD5(compressed bytes), but [dataDecoded] / `getAttachment` always return
  /// the original uncompressed bytes. Matches CouchDB's `encoding` field from
  /// `att_encoding_info=true` responses.
  ///
  /// This field is **not** serialized to JSON (stripped by [AttachmentInfoRawHook])
  /// so it never appears in CouchDB protocol messages.
